...
  }
  onProgramSelected(){
    this.commonService.getStudentByProgramId("/attendance/students",3).subscribe((response: any) => {
...
    });
  ...
  onCourseSelected(){
    this.commonService.getTopicByCourseId("/attendance/topic",16).subscribe((response:any)=>{
...
  }
  ...
  deleteRow(index: number, key = '') {
    ...
    dialogRef.afterClosed().subscribe((res) => {
      ...
      this.commonService
      .deleteData(control.controls[index].value.id, '/attendance/delete-attendance')
      .subscribe(
        (res) => {
          console.log(res);
        },
