...
export class StudentComponent implements AfterViewInit, DoCheck {
  displayedColumns: string[] = ['action', 'studentCode', 'studentName', 'studentEmail'];
 ...
 async  ngOnInit() {
  ...
  applyFilter(event: Event) {
    ...
    const rows = (this.VOForm.get('VORows') as FormArray).controls;
    const filteredRows = rows.filter(item => {
      const name = item.get('studentName')?.value?.toLowerCase();
      return name && name.includes(filterValue);
    });
  
    const filteredFormArray = new FormArray(filteredRows);
  
    this.VOForm.setControl('VORows', filteredFormArray);
  ...
  SaveVO(VOFormElement: any, i: any) {
    ...
    this.buttonDisable = false;
      ...
      if (VOFormElement.get('VORows').at(i).value.id) {
        this.commonService
          .getStudentByProgramIdandBatchId(
            VOFormElement.get('VORows').at(i).value.id,
            '/update-batchId-student',
