...
export class CoursesComponent {
  ...
  deleteRow(index: number) {
    ...
    dialogRef.afterClosed().subscribe((res) => {
      if (res) {
        const control = this.VOForm.get('VORows') as FormArray;
          this.commonService
            .deleteData(control.controls[index].value.courseId, '/course/delete')
            .subscribe(
              (res) => {
                console.log(res);
              },
  ...
  editDescription(VOFormElement: any, i: any) {
    ...
    dialogRef.afterClosed().subscribe((res) => {
      if (res) {
        console.log(res);
       console.log( VOFormElement.get('VORows').at(i).get('courseDescription'));

        VOFormElement.get('VORows').at(i).get('courseDescription').patchValue(res);
      }
    });
  }
  ...
  SaveVO(VOFormElement: any, i: any) {
    this.buttonDisable = false;
      ...
      if (VOFormElement.get('VORows').at(i).value.id) {
        ...
        this.commonService
          .addData('/course/add', VOFormElement.get('VORows').at(i).value)
          .subscribe(
            (res) => {
              console.log(res);
            },
