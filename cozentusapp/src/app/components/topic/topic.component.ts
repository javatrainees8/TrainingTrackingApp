...
export class TopicComponent {
 ...
 async ngOnInit() {
    ...
    ELEMENT_DATA=await this.commonService.getList('/topic/all-topics').toPromise()
     this.VOForm = this.fb.group({
      VORows: this.fb.array(
        ELEMENT_DATA.map((val) =>
          this.fb.group({
            action: new FormControl('existingRecord'),
            topic_code: new FormControl(val.topic_code),
            ...
            topic_id: new FormControl(val.topic_id),
  ...
  deleteRow(index: number, key = '') {
    ...
    dialogRef.afterClosed().subscribe((res) => {
      const control = this.VOForm.get('VORows') as FormArray;
      this.commonService
        .deleteData(control.controls[index].value.id, '/topic/delete')
        .subscribe(
          (res) => {
            console.log(res);
          },
  ...
  editDescription(VOFormElement: any, i: any, key = '') {
    const dialogRef = this.dialog.open(CommonDialogComponent, {
      data: {
        title: key === 'description' ? 'topicSummary' : 'topicSummary',
  ...
  editDescription2(VOFormElement: any, i: any, key = '') {
    const dialogRef = this.dialog.open(CommonDialogComponent, {
      data: {
        title: key === 'description' ? 'content' : 'content',
        description: VOFormElement.get('VORows').at(i).get(key).value,
      },
      disableClose: true,
      width: '400px',
  ...
  initiateVOForm(): FormGroup {
    return this.fb.group({
      action: new FormControl('existingRecord'),
            topic_code: new FormControl(null),
